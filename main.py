...
class MyFrame(wx.Frame):
    def __init__(self):
        ...
        host_box.Add(
            wx.StaticText(panel, label="Host:"),
            0,
            wx.ALL | wx.CENTER,
            PADDING_MEDIUM,
        )
        ...
        port_input_box.Add(
            wx.StaticText(panel, label="Custom Port:"),
            0,
            wx.ALL | wx.CENTER,
            PADDING_MEDIUM,
        )
    ...
    def check_port(self, event):
        ...
        if self.result_grid.GetNumberRows() > 0:
            self.result_grid.DeleteRows(
                0,
                self.result_grid.GetNumberRows()
            )
        ...
        if not ports:
            wx.MessageBox(
                "Bitte mindestens einen Port auswählen!",
                "Hinweis"
            )
            return
        ...
        self.progress.SetRange(total_ports)
        ...
        for index, port in enumerate(ports, start=1):
            ...
            try:
                with socket.create_connection(
                    ...
                    self.result_grid.SetCellValue(
                        row,
                        1,
                        "OPEN"
                    )

                    self.result_grid.SetCellBackgroundColour(
                        row,
                        1,
                        wx.Colour(*COLOR_PORT_OPEN)
                    )
